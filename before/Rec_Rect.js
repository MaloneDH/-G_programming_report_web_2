let canvas = document.getElementById("canvas-top"); //canvas tag 

let ctx = canvas.getContext("2d"); //드로잉 컨텍스트 설정

let gameState = {                   //게임의 필요한 정보 
  rectPosX: 10,                     //플레이어 위치(x)
  rectPosY: canvas.height / 2 - 10, //플레이어 위치(y)
  rectVelocity: { x: 0, y: 0 },     //플레이어 방향
  playerSpeed: 0.5,                 //플레이어 속도
  enemyTimeout: 60,                 
  enemyTimeoutInit: 60,
  enemySpeed: 1,                    //블럭 속도
  enemies: [],                      //적
...
function update() { //게임 실행 
  ...
  for (let i = 0; i < gameState.enemies.length; ++i) {
    gameState.enemies[i].x -= gameState.enemies[i].velocity;              //적 위치 이동 
...
setInterval(update, 20); //0.02초마다 다시 키기 
